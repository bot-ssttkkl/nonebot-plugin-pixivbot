...
get_driver().on_startup(pixiv_api.initialize)


@get_driver().on_startup
async def do_refresh():
    result = await pixiv_api.refresh(config.pixiv_refresh_token)
    ...
    if result.refresh_token != config.pixiv_refresh_token:
        logger.warning("refresh token has been changed: " + result.refresh_token)
