...
class TestWatchHandler(FakeWatchmanMixin,
    ...
    async def test_handle_no_arg(self, fake_post_destination,
        ...
        except_msg = "当前订阅：\n" \
                     ...
                     "following_illusts user_id=65432\n" \
                     "\n" \
                     "命令格式：/pixivbot watch <type> <..args>\n" \
                     "示例：/pixivbot watch user_illusts <用户名>\n"

        context.require(fake_watchman).tasks[
            (post_dest.identifier, WatchType.user_illusts, frozendict({"user_id": 54321}))
        ] = WatchTask(
        ...
        context.require(fake_watchman).tasks[
            (post_dest.identifier, WatchType.following_illusts, frozendict({"user_id": 65432}))
        ] = WatchTask(
            type=WatchType.following_illusts,
            kwargs={"user_id": 65432},
            subscriber=post_dest.identifier
        )
    ...
    async def test_handle_following_illusts(self, fake_post_destination,
        ...
        await context.require(fake_pixiv_account_binder).bind("test", 1234, 54321)
...
class TestUnwatchHandler(FakeWatchmanMixin,
    ...
    async def test_handle_following_illusts(self, fake_post_destination,
        ...
        except_msg = f"取消订阅成功"
        context.require(fake_watchman).tasks[
            (post_dest.identifier, WatchType.following_illusts, frozendict({"user_id": 54321}))
        ] = WatchTask(
            type=WatchType.following_illusts,
            kwargs={"user_id": 54321},
            subscriber=post_dest.identifier
