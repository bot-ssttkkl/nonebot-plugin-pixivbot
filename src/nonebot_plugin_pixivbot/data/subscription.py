...
from .sql_common import insert, JSON
...
class SubscriptionOrm(Model):
    __tablename__ = "pixivbot_subscription"
...
class SubscriptionRepo(IntervalTaskRepo[Subscription]):
    ...
    async def _to_model(self, item: SubscriptionOrm) -> Subscription:
    ...
    async def delete_one(self, session: Session, code: str) -> Optional[Subscription]:
        ...
        async with use_pixivbot_session() as db_sess:
            ...
            await db_sess.commit()
            return await self._to_model(sub)
    ...
    async def delete_many_by_session(self, session: Session) -> Collection[Subscription]:
        ...
        async with use_pixivbot_session() as db_sess:
            ...
            await db_sess.commit()
            return [await self._to_model(x) for x in subs]
