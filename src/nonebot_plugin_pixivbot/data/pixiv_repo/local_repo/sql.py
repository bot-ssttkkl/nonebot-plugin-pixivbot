...
class SqlPixivRepo(LocalPixivRepo):
            ...
            try:
                async for cache_illust, illust in await session.stream(stmt):
                    ...
                    if illust is not None:
                        yield LazyIllust(illust.illust_id, illust.illust)
                    else:
        ...
        async with use_pixivbot_session() as session:
            ...
            for x in content:
                ...
                stmt = insert(IllustDetailCache).values(
                    illust_id=x.id, illust=x, update_time=metadata.update_time
                )
            ...
            for x in content:
                if isinstance(x, LazyIllust):
                    ...
                    x = x.content
                li.append(x)
            await self.local_tags.update_from_illusts(li)
        ...
        async with use_pixivbot_session() as session:
            ...
            for x in content:
                stmt = insert(UserDetailCache).values(
                    user_id=x.id, user=x, update_time=metadata.update_time
                )
                stmt = stmt.on_conflict_do_update(
    ...
    async def illust_detail(self, illust_id: int) \
        ...
        async with use_pixivbot_session() as session:
            ...
            if cache is not None:
                metadata = _extract_metadata(cache, False).check_is_expired(
    ...
    async def update_illust_detail(self, illust: Illust, metadata: PixivRepoMetadata):
        ...
        async with use_pixivbot_session() as session:
            ...
            if self.conf.pixiv_tag_translation_enabled:
                await self.local_tags.update_from_illusts([illust])
    ...
    async def image(self, illust: Illust, page: int = 0) -> AsyncGenerator[Union[bytes, PixivRepoMetadata], None]:
        ...
        async with use_pixivbot_session() as session:
            ...
            if cache is not None:
                metadata = _extract_metadata(cache, False).check_is_expired(self.conf.pixiv_download_cache_expires_in)
